...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    // });
    allocationObj?.map(
      (asset: { empId: string; assetId: number; allocationTime: string }) => {
        db('assets')
          ...
          .catch(error =>
            res
              .status(400)
              .json({
                error: `Error occured while trying to allocate asset: ${asset?.assetId}`
              })
          )
      }
    )
    ...
    db('assetallocation')
      ...
      .then(() => {
        db('assets')
          ...
          .where('assetId', assetId)
          .then(() => {
            res.status(200).json({ message: 'Asset deallocated Successfully!' })
          })
          .catch(error => res.status(400).json({ error }))
      })
      .catch(error => res.status(400).json({ error }))
